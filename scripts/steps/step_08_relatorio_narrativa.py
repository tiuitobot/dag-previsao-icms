...
def main() -> int:
    args = parse_common_args('Step 08 - narrativa do relatório final', needs_input=True)
    output_dir = ensure_dir(Path(args.output))
    input_dir = Path(args.input_dir)
    previsoes = load_json(input_file(input_dir, 'previsoes.json'))
    payload = {
        'step': '08-relatorio-narrativa',
        'status': 'stub',
    ...
    dump_json(output_file(output_dir, 'relatorio_narrativa.json'), payload)
    print('OK relatorio_narrativa.json gerado (stub)')
    return 0
